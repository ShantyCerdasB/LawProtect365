...
export class WorkflowTestHelper {
   ...
   */
  async updateEnvelope(envelopeId: string, updateData: any): Promise<{
    statusCode: number;
    data: any;
  }> {
    const event = await this.makeAuthEvent({
   ...
   * @returns Promise that resolves to the update response
   */
  async addSigners(envelopeId: string, signers: SignerData[]): Promise<{
    statusCode: number;
    data: any;
  }> {
    return this.updateEnvelope(envelopeId, { addSigners: signers });
  }

  /**
   * Remove signers from an envelope
   * @param envelopeId - ID of the envelope
   * @param signerIds - Array of signer IDs to remove
   * @returns Promise that resolves to the update response
   */
  async removeSigners(envelopeId: string, signerIds: string[]): Promise<{
    statusCode: number;
    data: any;
  }> {
    return this.updateEnvelope(envelopeId, { removeSignerIds: signerIds });
  }

  /**
   * Change signing order of an envelope
   * @param envelopeId - ID of the envelope
   * @param signingOrderType - New signing order type
   * @returns Promise that resolves to the update response
   */
  async changeSigningOrder(envelopeId: string, signingOrderType: string): Promise<{
    statusCode: number;
    data: any;
  }> {
    return this.updateEnvelope(envelopeId, { signingOrderType });
  }
