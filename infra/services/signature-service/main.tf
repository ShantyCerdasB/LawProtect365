...
locals {
  ...
  # Shared runtime environment variables for ALL Lambdas:
  # - Populated from root (var.shared_lambda_env)
  # - Includes non-sensitive defaults
...
#
# Purpose:
# - Serves as the public API for the sign-service
# - Integrates with the three Lambda functions
#
# Features:
# - CORS enabled
# - Access logs stored in CloudWatch with optional KMS encryption
...
module "sign_api" {
  ...
  routes = [
    ...
    {
      route_key          = "GET /documents/{id}/certificate"
      integration_uri    = module.lambda_certificate.lambda_function_arn
      integration_type   = "AWS_PROXY"
      authorization_type = var.enable_jwt_authorizer ? "JWT" : "NONE"
    }
  ]
...
# Merge provided environment variables with service-specific ones
locals {
