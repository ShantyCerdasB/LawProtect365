...
// Basic types for outbox operations
export interface BaseCursorPayload {
  [key: string]: string;
}

export interface DdbItemWithAudit {
  pk: string;
  sk: string;
  type: string;
  createdAt: string;
...
/**
 * Outbox list result
 * Result for outbox list operations
 */
export interface OutboxListResult {
  items: any[]; // Will be properly typed when imported
  cursor?: string;
  hasMore: boolean;
}
...
/**
 * Outbox count result
 * Result for outbox count operations
 */
