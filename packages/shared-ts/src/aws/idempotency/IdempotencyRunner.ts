...
import { ConflictError, ErrorCodes } from "../../index.js";

/**
 * Configuration options for IdempotencyRunner.
 */
export interface IdempotencyRunnerOptions {
  /** Default TTL in seconds for idempotency records. */
...
export class IdempotencyRunner {
  ...
  async run<R>(key: string, fn: () => Promise<R>, ttlSeconds?: number): Promise<R> {
    ...
    // Record exists and should be valid (TTL handled by store)
