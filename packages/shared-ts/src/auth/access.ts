...
/**
 * Default access decision combining role hierarchy boundary and fine-grained permissions.
 * super_admin → allow all; admin → ; others → require permission or scope.
 *
 * Branding note:
 * - `Permission` and `Scope` are branded strings. To compare with scopes, we de-brand into plain strings.
 *
 * @param subject Security context (roles, scopes, permissions).
