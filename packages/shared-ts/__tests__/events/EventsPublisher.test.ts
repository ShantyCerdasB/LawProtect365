...
describe('makeEventPublisher', () => {
  const makePorts = () => {
    const bus = { publish: jest.fn<Promise<void>, [any]>() };
    const outbox = {
  ...
  it('publishes each record and marks them dispatched (headers present only with traceId)', async () => {
    const { bus, outbox } = makePorts();
    ...
    // First record: headers undefined
    ...
    // Second record: headers include x-trace-id
    expect(bus.publish).toHaveBeenNthCalledWith(
      2,
      expect.objectContaining({
        id: 'b',
  ...
  it('marks a record failed when publish throws and continues with others', async () => {
    ...
    bus.publish.mockImplementation(async (env: any) => {
      if (env.id === 'bad') throw new Error('boom');
    });
    ...
    // Verify publish calls for each with expected headers behavior
    expect(bus.publish).toHaveBeenCalledTimes(3);
    expect(bus.publish).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'ok1', headers: undefined })
    );
    expect(bus.publish).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'bad', headers: { 'x-trace-id': 'trace-bad' } })
    );
    expect(bus.publish).toHaveBeenCalledWith(
      expect.objectContaining({ id: 'ok2', headers: undefined })
    );
  });
  ...
  it('marks failed with stringified non-Error values', async () => {
    ...
    const now = '2024-01-02T03:04:05.000Z';
    outbox.pullPending.mockResolvedValueOnce([
      { id: 'x', type: 'E', occurredAt: now, payload: {} },
    ]);
