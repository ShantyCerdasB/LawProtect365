...
export async function recordConsentWithToken(
  ...
  if (response.statusCode !== 200) {
    ...
    try {
      ...
      console.error('RecordConsentWithToken error:', response.statusCode, response.body);
    } catch {}
  }
  expect(response.statusCode).toBe(200);
}
...
export async function completeSigningWithToken(
  ...
  // Use provided URL or create PDF in S3 (LocalStack) for reliability
  const finalPdfUrl = pdfUrl || await createTestPdfInLocalStackS3(envelopeId);
  const testPdf = generateTestPdf();
  const pdfDigest = calculatePdfDigest(testPdf);
 ...
 */
export async function getEnvelopeStatus(envelopeId: string, _ownerToken: string): Promise<any> {
  // Use the container directly to get the envelope
  const container = getContainer();
  const envelopesQueriesPort = container.envelopes.queriesPort;
  
  const result = await envelopesQueriesPort.getById({ 
    envelopeId: envelopeId as any // Cast to EnvelopeId type
  });
...
export async function createSingleSignerFlow(
  ...
  // Create owner party keeping ownerEmail stable to satisfy tests; avoid cooldown via unique userId and slight delay
  ...
  // Use the party's email for the actor token to satisfy "only invited party can consent"
  await recordConsentForAuthenticatedUser(envelope.id, ownerParty.id, ownerToken);
  console.log('🔍 [FLOW DEBUG] Consent recorded');

  // No artificial delay; rely on service timestamps/order
  ...
  return {
    envelope,
    parties: [ownerParty],
    owner: { id: uniqueUserId, email: ownerEmail, token: ownerToken },
    invitedUsers: [],
    invitationTokens: {}
  };
}
...
export async function createTwoSignersFlow(
  ...
  // Add small delay to avoid cooldown issues (cooldown is now 1 second)
  ...
  // Add delay to ensure envelope state is consistent in DynamoDB
