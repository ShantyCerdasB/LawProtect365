...
export const createTestRequestContext = (overrides: {
  ...
  const sourceIp = overrides.sourceIp || '8.8.8.8'; // Use valid public IP
  ...
  return {
    ...
    authorizer: {
      ...
      actor: {
        userId: overrides.userId || 'test-user-123',
        email: overrides.email || 'test@example.com',
        // ❌ REMOVED: ip: sourceIp, - Cognito doesn't include IP in actor
        // ❌ REMOVED: userAgent: userAgent, - Cognito doesn't include userAgent in actor
        role: role, // ✅ Keep role for metrics
        roles: ['user'],
        scopes: []
      }
    }
...
export const createApiGatewayEvent = async (overrides: {
  ...
  return {
    ...
    requestContext: {
      ...
      // Ensure identity is preserved if provided in overrides
      ...(overrides.requestContext?.identity && { identity: overrides.requestContext.identity })
    },
...
export const createTestRequestContextWithAuth = (overrides: {
  ...
  return {
    identity: {
      sourceIp: overrides.sourceIp || '8.8.8.8', // Use valid public IP
    ...
    authorizer: {
      ...
      actor: {
        userId: userId,
        email: email,
        // ❌ REMOVED: ip: overrides.sourceIp || '192.168.1.100', - Cognito doesn't include IP in actor
        roles: roles,
        scopes: scopes
      }
    }
