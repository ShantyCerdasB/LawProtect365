...
async function updateEnvelopeStatusIfNeeded(
  ...
  if (invitedCount > 0 && envelope.status === ENVELOPE_STATUSES[0]) {
    try {
      ...
      if (partiesRepo) {
        const partiesResult = await (partiesRepo as any).listByEnvelope({ envelopeId });
        const hasSigner = partiesResult.items.some((p: any) => p.role === "signer");
        if (!hasSigner) {
...
export function makeRequestsCommandsPort(config: RequestsCommandsPortConfig): RequestsCommandsPort {
  ...
  return {
    async inviteParties(command: InvitePartiesCommand): Promise<InvitePartiesResult> {
      ...
      // Get and validate envelope
      let envelope = await validateAndGetEnvelope(envelopesRepo, command.envelopeId);
      ...
      // Reload envelope if status was changed to get the updated state
      if (statusChanged) {
        const updatedEnvelope = await envelopesRepo.getById(command.envelopeId);
        if (updatedEnvelope) {
