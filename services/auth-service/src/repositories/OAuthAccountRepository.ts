...
export class OAuthAccountRepository extends RepositoryBase<OAuthAccount, OAuthAccountId, OAuthAccountSpec> {
  ...
  async findByProviderAndAccountId(
    ...
    try {
      const account = await (tx || this.prisma).oAuthAccount.findFirst({
        where: {
  ...
  /**
   * Upsert OAuth account (create or update)
   * @param userId - The user ID
   * @param provider - The OAuth provider
   * @param providerAccountId - The provider account ID
   * @param tx - Optional transactional context
   * @returns OAuthAccount entity
   */
  async upsert(
    userId: string,
    provider: OAuthProvider,
    ...
    try {
      const account = await (tx || this.prisma).oAuthAccount.upsert({
        where: {
          provider_providerAccountId: {
            provider: provider as any,
            providerAccountId: providerAccountId
