/**
 * @fileoverview TestSetUserStatusAdminHandler - Test-specific handler for POST /admin/users/{id}:set-status endpoint
...
import { SetUserStatusBodySchema } from '../../../src/domain/schemas/SetUserStatusSchema';
import { UserRole } from '../../../src/domain/enums/UserRole';

export const testSetUserStatusAdminHandler = async (evt: any) => {
  try {
    ...
    const body = SetUserStatusBodySchema.parse(rawBody);
    ...
    const result = await useCases.setUserStatusAdminUseCase.execute(
      request.userId,
      { 
