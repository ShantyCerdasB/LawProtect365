...
describe('Get Envelopes By User Workflow', () => {
  ...
  describe('Complete Envelope Information', () => {
    it('should return complete envelope and signer information', async () => {
      ...
      const envelopeSigners = findEnvelopeSigners(getResponse.data.signers, 'complete1@example.com');
      expect(envelopeSigners).toBeDefined();
      expect(envelopeSigners).toHaveLength(2);

      const signer1 = findSignerByEmail(envelopeSigners, 'complete1@example.com');
      const signer2 = findSignerByEmail(envelopeSigners, 'complete2@example.com');
...
/**
 * Helper function to find envelope signers by email
 * @param signers - Array of signer arrays
 * @param email - Email to search for
 * @returns Array of signers for the envelope
 */
function findEnvelopeSigners(signers: any[][], email: string): any[] {
...
/**
 * Helper function to find a signer by email
 * @param signers - Array of signers
 * @param email - Email to search for
 * @returns Signer object or undefined
 */
function findSignerByEmail(signers: any[], email: string): any {
