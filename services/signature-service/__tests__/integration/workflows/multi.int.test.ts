...
// 🔧 MOCK HOISTED - Debe estar ANTES de cualquier import que toque ServiceFactory
const mockOutbox = {
  save: jest.fn(async (event: any, dedupId?: string) => {
    // Track the event for verification
    const envelopeId = event?.payload?.envelopeId;
    if (envelopeId) {
      console.log(`🔧 Mocked OutboxRepository.save called for envelope ${envelopeId}`);
    }
    return Promise.resolve();
  }),
  findByEnvelopeId: jest.fn().mockResolvedValue([]),
...
jest.mock('@lawprotect/shared-ts', () => {
  ...
  return {
    ...actual,
    EventServiceFactory: {
      ...actual.EventServiceFactory,
      createOutboxRepository: jest.fn(() => mockOutbox), // <- intercepta la creación
    },
  };
});

// 👇 IMPORTS DESPUÉS del jest.mock
...
describe('Multi-Signer Document Signing Workflow', () => {
  ...
  afterEach(() => {
    // Clear OutboxRepository mock events after each test
