...
import { mapAwsError, nowIso, TooManyRequestsError, ErrorCodes } from "@lawprotect/shared-ts";
import type { DdbClientLike } from "@lawprotect/shared-ts";
import type { RateLimitStore, RateLimitWindow, RateLimitUsage } from "./RateLimitStore";

/** Stable entity marker. */
const RATE_LIMIT_ENTITY = "RateLimit" as const;
 ...
 */
interface DdbRateLimitItem {
  pk: string;
  sk: string;
  type: typeof RATE_LIMIT_ENTITY;
  rateLimitKey: string;
  windowStart: number;
  windowEnd: number;
  currentUsage: number;
