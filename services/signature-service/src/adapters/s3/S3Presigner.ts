...
import type {
  S3Presigner as S3PresignerPort,
...
export interface S3PresignerOptions {
   ...
   * Applied when `input.acl` is not provided to {@link putObjectUrl}.
   */
  defaultAcl?: "private" | "public-read";

  /**
   * Default Cache-Control header for PUT operations.
   * Applied when `input.cacheControl` is not provided to {@link putObjectUrl}.
   */
  defaultCacheControl?: string;

  /**
   * Default KMS key ARN/ID for SSE-KMS on PUT operations.
   * Applied when `input.kmsKeyId` is not provided to {@link putObjectUrl}.
   */
 ...
 */
export class S3Presigner implements S3PresignerPort {
