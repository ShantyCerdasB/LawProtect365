 ...
 * @description DynamoDB implementation of the `AuditRepository` port.
 */
export class AuditRepositoryDdb implements AuditRepository {
  ...
  /**
   * @summary Creates a new AuditRepositoryDdb instance
   * @description Creates a new AuditRepositoryDdb instance with the specified configuration.
   * @param tableName - DynamoDB table name
   * @param ddb - Minimal DDB client from the container
   * @param indexes - Optional index names for audit queries
   */
  constructor(
    private readonly tableName: string,
    private readonly ddb: DdbClientLike,
    indexes?: {
      readonly byEnvelope?: string;
      readonly byId?: string;
    }
  ) {
    this.idxByEnvelope = indexes?.byEnvelope ?? DEFAULT_ENTITY_INDEX;
    this.idxById = indexes?.byId ?? DEFAULT_ID_INDEX;
  }

  /**
   * @summary Fetches a single audit event by id using the ID GSI
   * @description Fetches a single audit event by id using the ID GSI.
  ...
  /**
   * @summary Appends a new immutable audit event and computes a hash link
   * @description Appends a new immutable audit event and computes a hash link.
