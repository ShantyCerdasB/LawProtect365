...
import { GlobalPartyCreateInput } from "@/shared/contracts/repositories/parties/GlobalPartyRepositoryPort";
...
export class GlobalPartyRepositoryDdb {
  ...
  /**
   * @description Creates a new Global Party from GlobalParty entity.
   */
  async createFromGlobalParty(globalParty: GlobalParty): Promise<void> {
  ...
  /**
   * @description Creates a new Global Party from CreateInput (implements GlobalPartyRepositoryPort)
   */
  async create(input: GlobalPartyCreateInput): Promise<void> {
    ...
    await this.createFromGlobalParty(globalParty);
  }
  ...
  /**
   * @description Finds a party by email in the specified tenant (implements GlobalPartyRepositoryPort)
   */
  async findByEmail(input: { tenantId: string; email: string }): Promise<{ partyId: PartyId } | null> {
    const result = await this.searchByEmail({
      tenantId: input.tenantId,
      email: input.email,
      limit: 1,
    });
    ...
    if (result.items.length > 0) {
      // Cast to PartyId since the interface expects it
      return { partyId: result.items[0].id as PartyId };
    }
